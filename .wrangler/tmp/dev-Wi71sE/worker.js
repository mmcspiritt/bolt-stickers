...
// .wrangler/tmp/bundle-Zvmnbx/checked-fetch.js
...
// ../../.nvm/versions/node/v20.18.1/lib/node_modules/wrangler/templates/modules-watch-stub.js
...
// .wrangler/tmp/bundle-Zvmnbx/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-Zvmnbx/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
__name(cleanForFilename, "cleanForFilename");
var worker_default = {
    ...
    try {
      ...
      if (items[0].image_url) {
        try {
          ...
          const filename = `stickers/${formatDate(date)}-${items[0].shape}-${items[0].quantity}-${cleanForFilename(items[0].size)}.png`;
      ...
      const amount = Math.round(items[0].price * items[0].quantity * 100);
      ...
      const paymentIntent = await stripe.paymentIntents.create({
        ...
        return_url: `${env.CLIENT_URL || "http://localhost:5173"}/success`
      });
